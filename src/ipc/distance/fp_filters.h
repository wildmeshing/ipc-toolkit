...
/*
Like the one above, but this compares to an epsilon. Needed for almost-parallel check. The one argument
must be set to 1.0 and is required by the predicate generator because otherwise the expression is not homogeneous.
If reading this made you raise your eyebrows and you know a way to avoid this, be my guest.
*/
inline int cross_almost_null_3d_filter( const double* p0, const double* p1, const double* q0, const double* q1, double eps, double one=1.0) {
	double v_0;
	v_0 = (p1[0] - p0[0]);
	double v_1;
	v_1 = (p1[1] - p0[1]);
	...
	double eps_FFWKCAA;
	double_tmp_result = (cross - (((one * one) * one) * eps));
	double max1 = fabs(eps);
	if( (max1 < fabs(w_0)) )
	{
		max1 = fabs(w_0);
	}
	...
	if( (lower_bound_1 < 3.43410674006831810450e-74) )
	{
		return FPG_UNCERTAIN_VALUE;
	}
	else
	{
		...
		eps_FFWKCAA = (1.59988686073108629406e-14 * (((max2 * max3) * max2) * max1));
		if( (double_tmp_result > eps_FFWKCAA) )
		{
			int_tmp_result = 1;
		}
		else
