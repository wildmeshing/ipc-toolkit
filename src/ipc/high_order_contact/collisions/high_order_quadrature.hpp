...
public:
    ...
    {
        if (n < 1) throw std::runtime_error("Order must be at least 1");
...
inline void lobatto_set(int n, std::vector<double> & xtab, std::vector<double> & weight)
...
*/
  ...
  do
  {
    ...
    for ( j = 2; j <= n-1; j++ )
    {
      for ( i = 0; i < n; i++)
      {
        p[i+j*n] = ( static_cast<double>( 2 * j - 1 ) * x[i] * p[i+(j-1)*n]
  ...
  for ( i = 0; i < n; i++ )
  {
    w[i] = 2.0 / ( static_cast<double>( ( n - 1 ) * n ) * pow ( p[i+(n-1)*n], 2 ) );
  }
}
