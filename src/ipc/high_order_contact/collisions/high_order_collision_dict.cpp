        ...
        std::shared_ptr<HighOrderCollision>>& map)
{
    ...
    for (const auto& [key, val] : map) {
        ...
        case HighOrderCollisionType::VERTEX_VERTEX:
            if constexpr (DIM == 2) {
                auto ptr = std::dynamic_pointer_cast<
                    HighOrderCollisionTemplate<Vertex2, Vertex2>>(val);
                assert(ptr);
                vv_collisions.push_back(*ptr);
            } else {
                auto ptr = std::dynamic_pointer_cast<
                    HighOrderCollisionTemplate<Vertex3, Vertex3>>(val);
                assert(ptr);
                vv_collisions.push_back(*ptr);
            }
            break;
        case HighOrderCollisionType::EDGE_VERTEX:
            if constexpr (DIM == 2) {
                auto ptr = std::dynamic_pointer_cast<
                    HighOrderCollisionTemplate<Vertex2, Edge2P1>>(val);
                assert(ptr);
                ev_collisions.push_back(*ptr);
            } else {
                auto ptr = std::dynamic_pointer_cast<
                    HighOrderCollisionTemplate<Edge3P1, Vertex3>>(val);
                assert(ptr);
                ev_collisions.push_back(*ptr);
            }
            break;
        case HighOrderCollisionType::FACE_VERTEX:
            if constexpr (DIM == 3) {
                auto ptr = std::dynamic_pointer_cast<
                    HighOrderCollisionTemplate<Face3P1, Vertex3>>(val);
                assert(ptr);
...
template <PointType pType, int DIM>
const HighOrderCollision&
HighOrderCollisionDict<pType, DIM>::operator[](int i) const
{
...
HighOrderCollisionDict<pType, DIM>::vertex_ids() const
{
    return m_vertex_ids;
}
...
template <PointType pType, int DIM>
const std::vector<index_t>&
HighOrderCollisionDict<pType, DIM>::primary_dofs() const
{
