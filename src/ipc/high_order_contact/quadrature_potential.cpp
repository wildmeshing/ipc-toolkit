        ...
        const index_t vid) const
    {
        ...
        for (const auto& other_f : f_set) {
            if (std::shared_ptr<HighOrderCollision> pair = HighOrderCollisionsBuilder<
                ...
                params, mesh, V)) {
                if (pair->is_active())
                    pairs.insert_pair(std::move(pair));
            }
        }
        ...
        const index_t e1) const
    {
            ...
            case PointEdgeDistanceType::P_E0:
                {
                    ...
                    pair2->weight = -1;
                    pairs.insert_pair(std::move(pair2));
                    break;
                }
        ...
        for (index_t other_f = 0; other_f < mesh.num_faces(); ++other_f) {
            ...
            case PointTriangleDistanceType::P_T0:
                {
                    pairs.insert_pair(std::shared_ptr<HighOrderCollision>(
            ...
            case PointTriangleDistanceType::P_T:
                {
                    pairs.insert_pair(std::shared_ptr<HighOrderCollision>(pair));
                    break;
                }
            default:
                assert(false);
