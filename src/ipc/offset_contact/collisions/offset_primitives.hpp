...
/**
 * @brief Base class for primitives used in offset contact models.
 *
 * This class defines the common interface for geometric primitives (like
 * vertices and edges) involved in a offset contact. Derived classes
 * are responsible for implementing the specific logic for their geometry type.
 */
class OffsetPrimitive {
public:
    OffsetPrimitive(const index_t id)
        : m_id(id)
    {
    }

    virtual ~OffsetPrimitive() = default;

    bool operator==(const OffsetPrimitive& other) const
    {
        return id() == other.id();
    }
