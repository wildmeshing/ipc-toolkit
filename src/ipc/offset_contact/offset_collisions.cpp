...
void OffsetCollisions::compute_adaptive_dhat(
    ...
    const bool use_adaptive_dhat)
{
    ...
    */
    ...
    if (mesh.dim() == 2) {
        auto storage = create_thread_storage<OffsetCollisionsBuilder<2>>(
            OffsetCollisionsBuilder<2>());
        maybe_parallel_for(
            candidates.ev_candidates.size(),
            [&](int start, int end, int thread_id) {
                OffsetCollisionsBuilder<2>& local_storage =
        ...
        OffsetCollisionsBuilder<2>::merge(storage, *this);
    } else {
        throw std::logic_error("Not implemented");
        /*
        auto storage = create_thread_storage<OffsetCollisionsBuilder<3>>(
            OffsetCollisionsBuilder<3>());
        maybe_parallel_for(
            candidates.ee_candidates.size(),
            [&](int start, int end, int thread_id) {
                OffsetCollisionsBuilder<3>& local_storage =
                    get_local_thread_storage(storage, thread_id);
        ...
        OffsetCollisionsBuilder<3>::merge(storage, *this);
        */
    }
...
// ============================================================================
size_t OffsetCollisions::size() const { return collisions.size(); }
bool OffsetCollisions::empty() const { return collisions.empty(); }
void OffsetCollisions::clear() { collisions.clear(); }

OffsetCollision& OffsetCollisions::operator[](size_t i)
{
...
const OffsetCollision& OffsetCollisions::operator[](size_t i) const
{
    if (i < collisions.size()) {
        return *collisions[i];
    }
...
std::string OffsetCollisions::to_string(
    const CollisionMesh& mesh,
    Eigen::ConstRef<Eigen::MatrixXd> vertices,
...
double OffsetCollisions::compute_minimum_distance(
...
double OffsetCollisions::compute_active_minimum_distance(
    const CollisionMesh& mesh, Eigen::ConstRef<Eigen::MatrixXd> vertices) const
{
    assert(vertices.rows() == mesh.num_vertices());
