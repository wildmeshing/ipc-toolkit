...
/*
TEST_CASE("Offset barrier potential codim", "[offset_potential]")
{
    ...
    OffsetContactPotential potential(params);
...
std::string tagsopt_ho = "[offset_potential]";
#else
std::string tagsopt_ho = "[.][offset_potential]";
#endif

TEST_CASE("Offset barrier potential full gradient and hessian 3D", tagsopt_ho)
{
    const auto method = make_default_broad_phase();
...
TEST_CASE("Offset barrier potential real sim 2D C^2", "[offset_potential]")
{
    double dhat = -1;
    Eigen::MatrixXd vertices;
    Eigen::MatrixXi edges;
    ...
    */
    ...
    SECTION("vertical_squares")
    {
        ...
        vertices.resize(8, 2);
        edges.resize(8, 2);
        vertices << // NOTE had to add a small offset to this, perfect alignment causes problems with alpha=0.
            -1.0001, 1.,
            -1.0001, 0.,
            -.1001, 0.,
            -.1001, 1.,
    ...
    test_offset_potential(vertices, edges, dhat);
}

TEST_CASE("Offset barrier potential real sim 2D C^1", "[offset_potential]")
{
    const auto method = make_default_broad_phase();
    //const bool adaptive_dhat = GENERATE(true, false);
    const bool adaptive_dhat = false;
